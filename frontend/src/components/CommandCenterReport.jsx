...
export default function CommandCenterReport({ project, compliance, session }) {
  ...
  const qrData = encodeURIComponent(`Vametra AI Trade Report ${reportId} | ${q.exporter?.name || ""}->${q.importer?.name || ""} | HS ${q.hsCode || p.hs || ""}`);
    ...
    <div id="cc-doc-print" style={{ display: "none" }}>
      <div style={{ fontFamily: "Arial, sans-serif", color: "#0A2540", padding: 8 }}>
        ...
        <div style={{ marginTop: 18, fontSize: 9, color: "#94a3b8", borderTop: "1px solid #e2e8f0", paddingTop: 6 }}>
          Report ID {reportId} · Generated {today}. Sources: {(q.sources || []).join(" · ")}. Deterministic engines compute all figures; the Vametra AI Brain interprets — it never fabricates numbers. Figures are indicative for planning; verify duty, tax and freight at the time of shipment.
          Generated by Vametra AI Trade Command Center · vametra.com
        </div>
      </div>
    </div>
  );
