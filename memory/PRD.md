...
## Fix — Users tab was missing most registered users (June 2026)
**Symptom:** `vaibhav.deshmane@vametra.com` (Customer ID 00002) and others were absent; the tab showed only 3 users.
**Root cause:** `GET /api/admin/users` (and allocation categories) built its list from the website-local Mongo `users` collection, which is only a partial mirror. The canonical registry is the DO identity backend (`GET {AUTH_API_BASE}/admin_v2/users`) — it holds all 11 registered users. Anyone who signed up through Firebase/DO but had no local row was invisible, and the local mirror even carried stale Customer IDs.
**Fix (subadmin.py):**
- `_fetch_do_users(authorization)` pulls the canonical registry with the main admin's Firebase token and mirrors it into `do_users_cache` (explicitly a READ-THROUGH CACHE, never an identity source).
- `_all_platform_users()` = live DO registry (or the cache when a sub-admin without a Firebase token is calling) MERGED with any website-local `users` rows not present upstream, so nothing can ever be dropped.
- Used by both `/admin/users` and `/admin/allocate/categories`; search now also matches uid and user_role.
