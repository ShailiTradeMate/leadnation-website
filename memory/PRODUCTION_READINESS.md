...
| Cookie consent (GDPR/CCPA) | ✅ DONE | Consent banner (Accept All / Reject Non-Essential / Manage Preferences); Essential always-on, Analytics + Marketing opt-in; GA4/GTM/Clarity/Meta load ONLY after consent; preference stored locally; "Cookie Preferences" link in footer to change anytime. First-party anonymous `/api/track` stays (privacy-scrubbed). |
| Analytics placeholders | ✅ INSTRUMENTED | Env-driven GTM/GA4/Clarity/Meta via reusable `analytics.js`; consent-gated; standard EVENTS wired; privacy scrubber blocks PII; first-party `/api/track`→`db.events` for future admin dashboard. See ANALYTICS.md. |
